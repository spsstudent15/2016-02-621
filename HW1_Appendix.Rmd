...
The table displayed below presents the results of our selected linear model's TARGET_WINS estimates for the 259 records contained in the  Evaluation data set:
...
# display all data
kable(eval.w, align = 'l', padding = 0)
...
boxplot(mb7, xlab="Boxplot Predictor Comparitive") 
...
test.1 <- lm(data=mb8, TARGET_WINS~BSR+TEAM_FIELDING_DP+TEAM_FIELDING_E+TEAM_BASERUN_SB+TEAM_PITCHING_SO+TEAM_PITCHING_H+TEAM_PITCHING_BB)
summary(test.1)
vif(test.1)
...
```{r full, eval = FALSE,include=TRUE, echo=TRUE}
...
```{r m1-iteration1,eval=FALSE,include=TRUE}
#VARIABLES
...
```{r m1-interation2,eval=FALSE,include=TRUE}
#VARIABLES
#variables have been transformed first as individual predictors
Wins  <- mW
p.Hits <- tmp.H 
...
#####Removed p.Hits
```{r m1-interation3,eval=FALSE,include=TRUE}
#VARIABLES
#variables have been transformed first as individual predictors
Wins  <- mW
p.Walks <- tmp.BB
...
```{r m1-interation4,eval=FALSE,include=TRUE}
nrow(lm.smk)
#VARIABLES
#variables have been transformed first as individual predictors
Wins  <- mW
...
```{r m1-interation5,eval=FALSE,include=TRUE}
#VARIABLES
#variables have been transformed first as individual predictors
Wins  <- mW
p.Walks <- tmp.BB
...
#####Remove b.Walks
```{r m1-interation6,eval=FALSE,include=TRUE}
#VARIABLES
#variables have been transformed first as individual predictors
Wins  <- mW
p.Walks <- tmp.BB
...
```{r m1-interation7,eval=FALSE,include=TRUE}
nrow(lm.smk)
#VARIABLES
#variables have been transformed first as individual predictors
Wins  <- mW
