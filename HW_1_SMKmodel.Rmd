...
#####Eliminate ahistorical outliers - DO THIS FOR THE EVAL DATA AS WELL
...
lm.smk <- lm.smk[-c(1737,1920,226,391,385,1702,840,602,1928,1937,2109,2128,269,711,125,2124,814,1624),]
# now renumber rows of dataframe so that there are no gaps in row numbers
...
#no transformation, used in Slugging
tmb.3B <- mb.3B#^(-1/33)
...
##TRANSFORMATION
powerTransform(mb.SL, family="bcPower")$lambda# round(0.5663934 ) => y^(3/5)
tmb.SL <- mb.SL^(3/5)
...
#no transformation, used in Fielding Yield
tmf.E <- mf.E#^(-(9/10))
...
box-cox method used with g(x) = y^(-1/25)
...
m1 <- lm(Wins ~ -Index+p.Hits+p.Walks)
...
m1 <- lm(Wins ~ -Index+p.Hits+p.Walks+b.Singles+b.Slugging)
...
m1 <- lm(Wins ~ -Index+p.Hits+p.Walks+b.Singles+b.Slugging+b.StolenBases+f.Fielding)
...
#DIAGNOSTIC1. show collinearity of variables after checking p-values to < 0.05.
vif(m1)
#p-values for p.Hits > 0.05 so remove it
```
...
m1 <- lm(Wins ~ p.Walks+b.Singles+b.Slugging+b.StolenBases+f.Fielding)
...
b.Walks <- tmb.BB
p.Strikeouts <-tmp.SO
m1 <- lm(Wins ~ -Index+p.Walks+b.Singles+b.Slugging+b.StolenBases+f.Fielding+b.Walks+p.Strikeouts)
...
#DIAGNOSTIC1. show collinearity of variables after checking p-values to < 0.05.
vif(m1)
#p-values are all < 0.05 and but b.Walks VIFs  > 5 for so remove it
```
...
\widehat{wins} &= \hat{\beta}_0 + 
                    ...
                    \hat{\beta}_3 \times b.Slugging +
                    \hat{\beta}_4 \times b.Stolen Bases +                    
                    \end{aligned}
\]
\[
\begin{aligned}
                    \hat{\beta}_5 \times b.Fielding Yield +  
                    \hat{\beta}_6 \times p.Strike Outs +
                    \end{aligned}
\]

#####Remove b.Walks
```{r m1-interation6,eval=TRUE,include=TRUE}
#VARIABLES
#variables have been transformed first as individual predictors
Wins  <- mW
Index <- lm.smk$INDEX
...
f.Fielding <- tmf.FY
p.Strikeouts <-tmp.SO
m1 <- lm(Wins ~ -Index+p.Walks+b.Singles+b.Slugging+b.StolenBases+f.Fielding+p.Strikeouts)
...
plot(b.StolenBases,StanRest,ylab="Standardized Residuals")
plot(f.Fielding,StanRest,ylab="Standardized Residuals")
plot(p.Strikeouts,StanRest,ylab="Standardized Residuals")
plot(m1$fitted.values,StanRest,ylab="Standardized Residuals",xlab="Fitted Values")
#Examine plots for constant variability of residuals across ALL predictor. 
# uniform distribution of residuals
...
```{r, eval=TRUE, include=FALSE}
#predict(m1, eval_data,interval="predict")
