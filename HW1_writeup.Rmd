...
plot.df <- mb_train[,2:17]

ggplot(stack(plot.df), aes(x = ind, y = values)) +
  geom_boxplot() +
  theme(axis.title = element_text(face="bold", size = 6),
        axis.text.x = element_text(angle = 90, hjust = 1)) +
  ggtitle("Boxplot Predictor Comparative")
...
1. __TEAM_BATTING_SO__: The adjusted $R^2$ value for this regression model is 0.7223 and yields a distribution matching the variables prior to the NA's replacement. For this predictor we impute a total of 102 missing values via regression.

2. __TEAM_PITCHING_SO__: The adjusted $R^2$ value for this regression model is 0.9952. We impute 102 missing values for pitching strikeouts.
...
4. __TEAM_FIELDING_DP__: The adjusted $R^2$ for this model is 0.3904. We impute 286 missing values with a similar distribution to the previous data.
...
plot.df <- mb_clean[,2:14]

ggplot(stack(plot.df), aes(x = ind, y = values)) +
  geom_boxplot() +
  theme(axis.title = element_text(face="bold", size = 6),
        axis.text.x = element_text(angle = 90, hjust = 1)) +
  ggtitle("Boxplot Predictor Comparative After Imputation + Outlier Removal")
...
Our selected model appears to performs well relative to the training data set we were provided. Though the training data set presented a variety of challenges, including signifcant outliers and missing data, we were able to address those challenges through the creation of a new derived variable (TEAM_BATTING_1B, or singles), the removal of three data fields (TEAM_HBP, TEAM_BASERUN_CS, and TEAM_PITCHING_HR), the removal of 104 records due to extreme outliers, and the imputation of the remaining missing values via linear regression models.
