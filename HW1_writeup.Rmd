...
In the process of building each model we run analysis to ensure that there are no collinearity issues and all p-values are < $.05$. Each model prodices imputed distributions of the subject variables that are consistent with those of the original NA-populated data. It is our belief that this consistency indicates that the resulting predicted values for the missing values are an improvement over simply filling the NA's with a mean or median.  The replacement of the NA's with numerical values allow us to run our final models on all records, not just those without NA's.  For consistency we will use the same approach with the evaluation data.
...
As result of this research, we feel confident in removing 104 lines with eggregious outliers that are impossible from a baseball historical perspective.  Using this SME knowledge will normlize our data and improve the expected performance of our linear models.

## Results of Imputation for Missing Values and Outlier Removal Process

The charts below show that our data transformation process is dramatically improving the data,  There still are a few outliers but on a dramatically smaller scale, dramatically reducing the range especially for the pitching values.
...
#####Conclusion:
This model is an improvement over the first TB_PLUS model when the residual plots are considered, and the number of predictor variables used is two fewer than that of the "Total Bases" model.  Just like in model 2, the coefficient for TEAM_FIELDING_E changes from negative to positive. Overall, there is a lot to like about this model,
__--------------------------------------------------------------------------------------------------------------------------__
...
Of the remaining two, the Sabermetrics model yields a slightly large $R^2$ value showing a slightly better possible fit.  However, the TB Plus model is simpler it make as it uses only 4 predictor variables while possessing a much larger F-statistic. Such a large difference in F-statistics indicates that the TB Plus model is explaining more of the variability of the training data than is the Sabermetrics model.  Therefore, __we select the TB PLUS model__ as the basis for our prediction of TARGET_WINS for the Evaluation data set. 

#####Step 4:  Apply to the evaluation data
To ensure the model's efficacy when applied to the Evaluation data set, we first apply a set of transformations to that data set identical to those applied to the Training data set during the development of the TB PLUS model. The TB PLUS model is then applied to yield a set of INDEX / TARGET_WINS pairs.

Since displaying the full set of predicted values would consume a large number of pages, a sample of the first 10 rows of that data set is displayed as an example:
